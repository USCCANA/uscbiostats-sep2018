...
1.  _"SAS is uglier~~easier~~ than R"_

2.  _"SAS is **NOT** required for drug tests by the FDA"_

3.  _"R **is cool** ~~... but it can't~~ **AND CAN** handle data out-of-memory like SAS"_

4.  _"SAS ~~has a higher demand than R in~~ **IS LEAVING** the job market"_
...
simgraph <- function(n, p) {
  ...
  ans<-replicate(n, netdiffuseR::rewire_graph(x, p, both.ends = TRUE))  %>%
    statistic()
  ans$score
}
...
set.seed(2123)
library(magrittr)
ans <- parallel::mcmapply(function(n, p) {
  data.frame(
    size = n,
    prob = p,
    stat = replicate(nsims, tryCatch(simgraph(n, p), error=function(e) e))
  )
  }, n = combinations$n, p=combinations$p, mc.cores=4L, SIMPLIFY = FALSE,
  mc.set.seed = TRUE
  )
```

```{r plot-experiments, out.width="600px"}
...
```{r plot-experiments2, out.width="600px"}
